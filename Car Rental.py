...
for i, (car_name, img_file) in enumerate(car_list):
    ...
    chk = tk.Checkbutton(frame, text=car_name, variable=var, image=car_images[i],
                         compound='left', padx=10, bg='white', command=lambda i=i: toggle_car(i),
                         font=("Arial", 10))
    chk.pack(anchor='w', pady=4)

tk.Button(app, text="Save Rental", bg="green", fg="white", command=save_rental).pack(pady=10)
tk.Button(app, text="View Rentals", command=view_rentals).pack()
